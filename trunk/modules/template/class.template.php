...
class template extends module {
    ...
    // auto_submit for templates
        ...
        $sql = "select template_key, template_name from m_template order by template_name";
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                $ret_val .= "<select name='template' size='5' class='textbox' onchange='this.form.submit();'></select>";
    ...
    function form_template() {
        ...
        print "<textarea class='textbox' rows='10' cols='40' name='template_text'style='border: 1px solid #000000'>".($template["template_text"]?$template["template_text"]:$post_vars["template_text"])."</textarea><br>";
        print "</td></tr>";
        print "<tr><td><br>";
        if ($get_vars["template_key"]) {

...
class healthcenter extends module{
    ...
    //
        ...
        module::set_lang("FTITLE_CONSULT_STATS", "english", "CONSULT STATS", "Y");
        module::set_lang("LBL_PATIENT_COUNT", "english", "PATIENT COUNT", "Y");
        module::set_lang("LBL_CONSULT_DETAILS", "english", "CONSULT DETAILS", "Y");
        module::set_lang("THEAD_REGN_PERIOD", "english", "REGISTRATION PERIOD", "Y");
        module::set_lang("THEAD_PATIENTS", "english", "NO. OF PATIENTS", "Y");
        module::set_lang("THEAD_CONSULT_PERIOD", "english", "CONSULT PERIOD", "Y");
        module::set_lang("THEAD_CONSULTS", "english", "NO. OF CONSULTS", "Y");
        module::set_lang("THEAD_AVGTIME", "english", "AVE CONSULT TIME", "Y");
    ...
    function init_menu() {
        ...
        module::set_menu($this->module, "Consult Stats", "CONSULTS", "_consult_stats");
    ...
    //
    // display health center statistics
    //
        ...
        print "<span class='patient'>".FTITLE_CONSULT_STATS."</span><br/><br/>";
        print "<table width='600'>";
        print "<tr valign='top'><td width='50%'>";
        // column 1
        $sql = "select count(patient_id) count, monthname(registration_date) month, ".
               "year(registration_date) year ".
               "from m_patient ".
               "where year(registration_date) = year(sysdate()) ".
               "group by month(registration_date)";
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                print "<b><font color='red'>".LBL_PATIENT_COUNT."</font></b><br><br>";
                print "<table width='250' bgcolor='black' cellspacing='1'>";
                print "<tr bgcolor='lightgrey'><td><b>".THEAD_REGN_PERIOD."</b></td><td><b>".THEAD_PATIENTS."</b></td></tr>";
                while ($patient = mysql_fetch_array($result)) {
                    print "<tr bgcolor='white'><td>".$patient["month"]." ".$patient["year"]."</td>";
                    print "<td>".$patient["count"]."</td></tr>";
                }
                print "</table>";
            } else {
                print "<font color='red'>No patients for selected year.</font><br>";
            }
        }
        print "</td><td>";
        // column 2
        $sql = "select count(consult_id) count, round(avg(elapsed_time),2) avgtime, ".
               "monthname(consult_end) month, year(consult_end) year ".
               "from m_consult ".
               "where year(consult_end) = year(sysdate()) ".
               "group by month(consult_end)";
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                print "<b><font color='red'>".LBL_CONSULT_DETAILS."</font></b><br><br>";
                print "<table width='300' bgcolor='black' cellspacing='1'>";
                print "<tr bgcolor='lightgrey'><td><b>".THEAD_CONSULT_PERIOD."</b></td><td><b>".THEAD_CONSULTS."</b></td><td><b>".THEAD_AVGTIME."</b></td></tr>";
                while ($consult = mysql_fetch_array($result)) {
                    print "<tr bgcolor='white'><td>".$consult["month"]." ".$consult["year"]."</td>";
                    print "<td>".$consult["count"]."</td>";
                    print "<td>".$consult["avgtime"]."</td></tr>";
                }
                print "</table>";
            } else {
                print "<font color='red'>No consults for selected year.</font><br>";
            }
        }
        print "</td></tr>";
        print "</table>";
    ...
    function vitals_detail() {
        ...
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                ...
                print "HPN STAGE: ".healthcenter::hypertension_stage($syst, $diast)."<br/>";
                print "</td></tr>";
                print "<tr><td colspan='2'>";
                if ($_SESSION["priv_delete"]) {
    ...
    function get_lastvisit() {
        ...
        $sql = "select date_format(consult_date, '%a %d %b %Y, %h:%i%p') from m_consult where patient_id = '$patient_id'";
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                list($date) = mysql_fetch_array($result);
                return $date;

...
class database extends module {
    ...
    //
        ...
        module::set_lang("MENU_LIBDUMP", "english", "LIBRARY DUMP", "Y");
    ...
    function _database_support() {
        ...
        if ($get_vars["db_menu"]) {
            ...
            case "BACKUP":
                ...
                break;
            case "LIBDUMP":
                $d->library_dump($menu_id, $post_vars, $get_vars);
            }
        }
    }
    ...
    function database_menu() {
        ...
        print "<td><a href='".$_SERVER["PHP_SELF"]."?page=".$get_vars["page"]."&menu_id=".$get_vars["menu_id"]."&db_menu=LIBDUMP' class='ptmenu'>".($get_vars["db_menu"]=="LIBDUMP"?"<b>".MENU_LIBDUMP."</b>":MENU_LIBDUMP)."</a></td>";
        print "</tr></table>";
        print "<br/>";
    }
    
    function library_dump() {
        if (func_num_args()>0) {
            $arg_list = func_get_args();
            $menu_id = $arg_list[0];
            $post_vars = $arg_list[1];
        ...
        if ($post_vars["submitdb"]) {
            database::process_libdump($menu_id, $post_vars, $get_vars);
        }
        database::form_libdump($menu_id, $post_vars, $get_vars);
        print "</td><td>";
        // column 2
        database::display_libdump($menu_id, $post_vars, $get_vars);
        print "</td></tr></table>";
    }

    function form_libdump() {
        if (func_num_args()>0) {
            $arg_list = func_get_args();
            $menu_id = $arg_list[0];
            $post_vars = $arg_list[1];
        ...
        print "<b>LIBRARY DUMP</b><br/><br/>";
        print "You can dump (extract) data from library tables in SQL format. ".
              "The latest snapshot of library table is created and ".
              "named after the library table. ";
        print "<br/><br/>";
        print "<form method='post' action=''>";
        $sql = "show tables like '%m_lib_%'";
        if ($result = mysql_query($sql)) {
            if (mysql_num_rows($result)) {
                print "<select name='tablename' class='textbox'>";
        ...
        print "<br><br>";
        print "<input type='submit' name='submitdb' value='Dump Library Table' class='textbox' style='border: 1px solid black'/>";
        print "</form>";
    }
    
    function display_libdump() {
    //
    ...
    //
        ...
        // manage delete here
        //
        if ($post_vars["submitfile"]=="Delete Selected Files" && $post_vars["tbldump_files"]) {
            foreach ($post_vars["tbldump_files"] as $key=>$value) {
                @unlink("../dump/".$value);
            }
        }
        print "<b>LIBRARY TABLE DUMPS</b><br/><br/>";
        $d = dir("../dump");
        print "<span class='tinylight'>";
        print "<form method='post' action=''>";
        while (false !== ($entry = $d->read())) {
            if ($entry<>"." && $entry<>".." && ereg("m_lib_", $entry)) {
                print "<input type='checkbox' name='tbldump_files[]' value='$entry'/> ";
    ...
    //
        ...
        if (file_exists("/usr/bin/mysqldump") && $post_vars["tablename"]) {
            // backtick operator
            $user = $_SESSION["dbuser"];
            $pass = $_SESSION["dbpass"];
            $dbname = $_SESSION["dbname"];
            $tablename = $post_vars["tablename"];
            ...
            // --extended-insert
            `/usr/bin/mysqldump -u $user -p$pass -t -n --quick --extended-insert --single-transaction $dbname $tablename> ../dump/$filename`;
        }
    }
    ...
    //
        ...
        print "<form method='post' action=''>";
        while (false !== ($entry = $d->read())) {
            if ($entry<>"." && $entry<>".." && ereg("game", $entry)) {

...
class appointment extends module {
    ...
    function form_consult_appointment() {
        ...
        if ($get_vars["cid"] && $get_vars["injury_ts"]) {
            ...
            if ($_SESSION["priv_update"]) {
                print "<input type='submit' value = 'Update Schedule' class='textbox' name='submitsked' style='border: 1px solid #000000'> ";
            }
            if ($_SESSION["priv_delete"]) {
            ...
            if ($_SESSION["priv_add"]) {
                print "<br><input type='submit' value = 'Save Schedule' class='textbox' name='submitsked' style='border: 1px solid #000000'><br> ";
            }
        }
        print "</td></tr>";
        print "</form>";
    ...
    //
    // main method for appointment codes
    // calls form_appointment, process_appointment, display_appointment
    //
        if (func_num_args()>0) {
            $arg_list = func_get_args();
            $menu_id = $arg_list[0];
